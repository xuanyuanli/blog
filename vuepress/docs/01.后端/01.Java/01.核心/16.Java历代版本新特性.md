...
Java 由 Oracle 主导，通过 OpenJDK 社区协作开发，采用`时间驱动的发布模型（Time-Based Release Model）`。

#### 关键特点
- 每 6 个月发布一个新版本（3月和9月）
- 长期支持（LTS）版本每 2 年一次（如 Java 11、17、21、25……）
- 开发基于 JEP（JDK Enhancement Proposal）

#### 全流程步骤

##### 1. 提案与讨论（JEP 起草）
- 开发者或组织提交 JEP（类似 Python 的 PEP）
- JEP 描述新特性、动机、实现方案、兼容性影响等

##### 2. JEP 状态流转
Draft → Submitted → Candidate → Proposed to Target → Targeted（纳入某版本）
- 最终状态：Completed（合并进 JDK）

##### 3. 集成到 OpenJDK 代码库
- 由各 JDK 项目（如 Valhalla、Loom、Panama）在主线或特性分支开发
- 通过代码审查、CI 测试后合并

##### 4. 版本里程碑（Rampdown）
- Feature Freeze（功能冻结）
- Only critical bug fixes allowed
- 发布多个 Release Candidate (RC)

##### 5. 正式发布（GA - General Availability）
- 每年 3月和9月 固定发布（如 JDK 21 → 2023-09，JDK 22 → 2024-03）
- 非 LTS 版本仅维护到下一个版本发布（约6个月）
- LTS 版本由厂商（Oracle、Azul、Red Hat 等）提供多年支持

##### 6. 后续维护
- 安全更新、bug 修复通过 Update Releases（如 JDK 21.0.1, 21.0.2...）

#### 治理模型
- 代表组织：Oracle、OpenJDK 社区、Adoptium、Azul、Red Hat 等
- 治理模型：相对集中（Oracle 主导，社区协作）
...
**注意**：Java 24不是LTS版本，仅提供6个月的支持。

## 25（LTS）
2025年9月16日[发布](https://openjdk.org/projects/jdk/25/)。

Java 25是继Java 21后的新LTS版本，包含18个JEP特性，在开发者体验、性能优化和安全性方面有重要突破。

### 核心正式特性

* **JEP 506**: [作用域值（正式版）](/pages/scoped-values-structured-concurrency/)。提供线程局部存储的更好替代方案，支持在线程间安全共享不可变数据：
  ```java
  // 定义作用域值
...
* **JEP 510**: 密钥派生函数API（正式版）。提供标准化的密钥派生功能，支持HKDF等算法

* **JEP 511**: 模块导入声明（正式版）。简化模块依赖的导入方式：
  ```java
  import module java.base;  // 导入整个模块的所有导出包
...
* **JEP 512**: 紧凑源文件和实例主方法（正式版）。简化Java入门和脚本编写：
  ```java
  // 无需public class，无需static main
  void main() {
      System.out.println("Hello, Java 25!");
  }
  ```

* **JEP 513**: 灵活的构造函数体（正式版）。允许在`super()`调用之前执行验证代码：
  ```java
...
* **JEP 514**: AOT命令行工具改进。简化提前编译的使用方式
* **JEP 515**: AOT方法分析。预先分析方法行为以改善JIT编译性能，减少应用启动时间
* **JEP 519**: 紧凑对象头（正式版）。将对象头从128位压缩到64位，显著减少内存占用
* **JEP 521**: 分代Shenandoah（正式版）。为Shenandoah垃圾收集器添加分代支持，提升GC性能

### JFR增强

* **JEP 509**: JFR CPU时间分析（实验性）。提供更精确的CPU时间分析能力
* **JEP 518**: JFR协作采样。改进采样机制，减少性能开销
* **JEP 520**: JFR方法计时与追踪。增强方法级别的性能监控

### 预览和孵化特性

* **JEP 470**: PEM编码的加密对象（预览）。支持PEM格式的密钥和证书编解码
* **JEP 502**: 稳定值（预览）。提供延迟初始化的不可变值容器
* **JEP 505**: 结构化并发（第五次预览）。简化并发编程模型
* **JEP 507**: 原始类型模式匹配（第三次预览）。支持`int`、`long`等原始类型的模式匹配：
  ```java
  switch (value) {
      case int i when i > 0 -> System.out.println("正整数: " + i);
      case int i -> System.out.println("非正整数: " + i);
      case double d -> System.out.println("浮点数: " + d);
  }
  ```
* **JEP 508**: Vector API（第十个孵化器）。SIMD向量计算支持

### 平台变化

* **JEP 503**: 移除32位x86端口。正式停止支持32位x86架构

**升级建议**：作为LTS版本，Java 25提供至少5年的长期支持，是生产环境的稳定选择。建议从Java 21或更早版本升级的用户重点关注作用域值、紧凑源文件和灵活构造函数体等正式化特性。
