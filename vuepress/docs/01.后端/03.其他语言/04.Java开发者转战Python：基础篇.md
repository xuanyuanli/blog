...
如果f-string中本身要输出`{`或`}`，则需要使用`{{`和`}}`进行转义：
```python
text = "hello"
print(f"{{前缀}} 内容：{text}") # {前缀} 内容：hello
```
...
**扩展解包**
扩展解包（Extended Unpacking）是 Python 3 引入的重要语法特性，它使用 * 操作符在赋值（如元组/列表解包）或函数调用中灵活处理可变数量的元素。

```python
# 在赋值语句中，* 可以出现在目标序列（左边）的任意位置，用于收集“剩余”元素：
a, *rest, b = [1, 2, 3, 4, 5]
# a = 1, rest = [2, 3, 4], b = 5
...
print(first)   # 95
print(middle)  # [88, 92, 87]
print(last)    # 90

# 忽略中间部分（配合 _ 命名约定）
# 例如：处理 CSV 行，只关心第一列和最后一列
row = ["ID001", "张三", "男", "25", "北京", "active"]
user_id, *_, status = row
print(user_id, status)  # ID001 active
```
...
**`**` 的用法：字典解包与合并（Python 3.5+）**

在字典推导式或字面量构造中，`**` 可用于**解包字典**，常用于合并多个字典或动态构建新字典：

```python
# 合并两个字典（Python 3.5+）
defaults = {"theme": "dark", "lang": "zh"}
overrides = {"lang": "en", "notifications": True}
...
default_profile = {"status": "active", "verified": False}

# 为每个用户合并默认配置
enhanced_users = {user["name"]: {**default_profile, **user} for user in users}
# 结果：
# {
#   "Alice": {"status": "active", "verified": False, "name": "Alice", "role": "admin"},
#   "Bob":   {"status": "active", "verified": False, "name": "Bob",   "role": "user"}
# }

# 注意：** 只能解包字典（或实现了 __getitem__ 和 keys() 的映射对象）
```
...
```python
# 提示：装饰器是Python的重要特性，将在进阶篇"高级特性"中详细讲解
# 这里简要展示装饰器与闭包的关系
...
> **提示**：上下文管理器的完整用法（包括`@contextmanager`装饰器、`ExitStack`等）将在进阶篇的"高级特性"中详细讲解。
...
📚 **系列**：
- [Java开发者转战Python：进阶篇](/pages/python-for-java-dev-raise/) - 异常处理、文件操作、装饰器、异步编程
- [Java开发者转战Python：生态与质量管理](/pages/python-for-java-dev-package/) - Python生态、质量管理
