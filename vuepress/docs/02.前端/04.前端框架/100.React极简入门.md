...
理解 `useEffect` 是掌握 React 函数组件的关键一步。它用于处理"副作用"（side effects），这个术语听起来比较抽象，下面用更具体的语言来解释。

#### 什么是"副作用"？

在函数式编程中，**副作用**指的是：**函数执行过程中，除了返回值以外，对外部环境产生的任何影响**。比如：
...
这些操作会"影响外部世界"或"依赖外部状态"，所以不能在纯函数中直接做，而需要被"隔离"处理 —— React 就用 `useEffect` 来处理它们。
...
useEffect(() => {
  ...
  // 可选：返回一个"清理函数"
  return () => {
    console.log('组件卸载前或下次 effect 执行前清理');
  };
}, [/* 依赖数组 */]);
```

- **第一个参数**：副作用函数（执行你想要的操作）
- **第二个参数（可选）**：依赖数组，决定这个 effect 何时重新运行
- **返回值（可选）**：一个清理函数，用于取消订阅、清除定时器等，防止内存泄漏

#### 依赖数组

依赖数组用于控制 `useEffect` 的执行时机，有三种情况：
...
function EffectDependencies() {
  ...
  // 情况2：空依赖数组 - 只在挂载时执行一次（类似 componentDidMount）
  useEffect(() => {
    console.log('组件挂载了');
  }, []);

  // 情况3：有依赖项 - 依赖项变化时执行（类似 componentDidUpdate）
  useEffect(() => {
...
> "每当某些东西（依赖项）改变后，我想做点'额外的事'（副作用），并且可能需要在下次做之前先'收拾干净'。"

它把原本分散在 class 组件中的 `componentDidMount`、`componentDidUpdate`、`componentWillUnmount` 三种生命周期合并到了一个统一的逻辑单元中，并用依赖数组精细控制执行时机。

#### 清理函数

有些副作用需要清理，比如定时器、事件监听等。清理函数用于防止内存泄漏（类似 `componentWillUnmount`）。
...
##### 场景 1：组件挂载时只运行一次（类似 componentDidMount）
...
##### 场景 2：监听某个状态变化（类似 componentDidUpdate）
...
##### 场景 3：清理副作用（类似 componentWillUnmount）
...
- 如果你在 effect 中用了某个变量（比如 `userId`），记得把它加到依赖数组里
- 不要把"不需要副作用"的逻辑放进 `useEffect`，比如计算衍生状态 —— 那应该用 `useMemo`
- 开发环境下，React 可能会**故意**调用 effect 两次（严格模式），这是为了帮你发现缺少清理的问题
