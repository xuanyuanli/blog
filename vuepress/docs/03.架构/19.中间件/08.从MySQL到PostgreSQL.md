...
##### 6.3、multicorn2：新一代高级框架
Multicorn2 是 PostgreSQL 外部数据包装器（Foreign Data Wrapper，FDW）生态中的新一代高级框架，专为简化自定义 FDW 开发、适配现代 PostgreSQL 版本（包括 PG 18）设计，核心目标是让开发者无需编写复杂的 C 代码，仅通过 Python 就能实现 PostgreSQL 与任意外部数据源（如 Elasticsearch、MySQL、API 服务、CSV/Excel 等）的集成。

Multicorn2 是 PG 18 集成外部数据源的 “瑞士军刀”，核心价值是用 Python 降低 FDW 开发门槛，同时适配 PG 18 的新特性（异步、并行、JSONB 优化），尤其适合对接 Elasticsearch、API 等非传统数据源。对于需要快速实现 PG 与多源数据集成的场景，Multicorn2 是远优于原生 FDW 的选择，也是 PG 18 生态中外部数据集成的核心工具之一。
...
##### 7.3、pg_bigm专为CJK优化的2元组方案

**pg_bigm的技术优势**

`pg_bigm` 是专门为中日韩（CJK）语言设计的 PostgreSQL 扩展，使用 2元组（bigram）而非 3元组（trigram）来索引文本，更适合处理CJK字符：
...
SELECT 
    ...
    content
FROM articles
WHERE title LIKE '%PostgreSQL数据库%'
   OR content LIKE '%全文搜索%';

-- 相似度搜索
SELECT 
    title,
    pg_bigm.likequery(title, '数据库') as like_query,
    pg_bigm.similarity(title, 'PostgreSQL数据库') as sim_score
FROM articles
WHERE pg_bigm.similarity(title, 'PostgreSQL数据库') > 0.3
ORDER BY sim_score DESC
LIMIT 10;

-- 与pg_trgm对比：短文本搜索
-- pg_bigm对1-2个汉字的搜索效果更好
SELECT 
    id,
    title,
    pg_bigm.similarity(title, '数据') as bigm_sim,
    similarity(title, '数据') as trgm_sim
FROM articles
WHERE pg_bigm.similarity(title, '数据') > 0.2
ORDER BY bigm_sim DESC;
```

**pg_bigm vs pg_trgm：关键差异**

| 特性 | pg_bigm | pg_trgm | 最佳场景 |
|------|---------|---------|----------|
| **n-gram长度** | 2个字符 | 3个字符 | CJK选bigm，西文选trgm |
| **短文本支持** | 优秀（1-2字符） | 较差（需3+字符） | 中文搜索用bigm |
| **LIKE优化** | 直接支持 `LIKE '%关键词%'` | 仅支持相似度查询 | 需LIKE场景用bigm |
| **索引大小** | 相对较小 | 相对较大 | 存储敏感用bigm |
| **查询精度** | 较高（2-gram重叠更精确） | 中等（3-gram可能漏召回） | 精确搜索用bigm |
| **西文支持** | 一般 | 优秀 | 英文为主用trgm |
...
CREATE INDEX idx_products_desc_bigm ON products USING GIN(description gin_bigm_ops);
...
SELECT 
    id,
    name,
    pg_bigm.similarity(name, '苹果手机') as score
FROM products
WHERE name LIKE '%手机%'  -- 先过滤
  AND pg_bigm.similarity(name, '苹果手机') > 0.3  -- 再相似度排序
ORDER BY score DESC
LIMIT 20;
...
SELECT 
    ...
    description,
    GREATEST(
        pg_bigm.similarity(name, '华为Mate'),
        pg_bigm.similarity(description, '华为Mate'),
        pg_bigm.similarity(brand, '华为Mate')
    ) as max_score
FROM products
WHERE name LIKE '%华为%' 
...
```sql
-- 调整相似度阈值（默认0.3，范围0.0-1.0）
SET pg_bigm.similarity_limit = 0.2;  -- 降低阈值，召回更多结果
...
SHOW pg_bigm.gin_key_limit;  -- GIN索引键限制，默认0（无限制）

-- 持久化配置（修改postgresql.conf）
ALTER DATABASE your_database SET pg_bigm.similarity_limit = 0.25;
```

**pg_bigm的局限性**

虽然 `pg_bigm` 针对CJK优化显著，但仍有以下限制：

* **无语义理解**：和 `pg_trgm` 一样，只基于字符匹配，不理解词义（"苹果手机"和"手机苹果"得分相同）
* **长文本索引膨胀**：虽比 `pg_trgm` 小，但大文本列的 GIN 索引仍会占用大量空间
* **无分词能力**：不做词语切分，"数据库系统"会被切成"数据"、"据库"、"库系"等，可能产生误召回
* **依赖LIKE语法**：虽加速了 `LIKE '%keyword%'`，但复杂布尔查询（AND/OR/NOT）需自行组合SQL
* **排序算法简单**：相似度计算基于 bigram 重叠度，不如 BM25/TF-IDF 等专业搜索算法

**何时选择pg_bigm**

选择 `pg_bigm` 的典型场景：

1. **中文为主的应用**：电商商品搜索、中文知识库、中文内容平台
2. **需要LIKE加速**：现有代码大量使用 `LIKE '%keyword%'`，不想改写查询
3. **短文本/短关键词**：用户常搜索1-2个汉字（如"数据"、"搜索"）
4. **轻量级需求**：不需要 Elasticsearch 级的复杂功能，PostgreSQL 扩展足够
5. **与其他扩展组合**：可以和 `tsvector/tsquery`、`zhparser`、`pg_trgm` 等并存，各司其职

```sql
-- 推荐组合方案：pg_bigm + tsvector
-- tsvector处理分词和权重，pg_bigm处理模糊匹配
CREATE TABLE articles (
    id SERIAL PRIMARY KEY,
    title TEXT,
    content TEXT,
    search_vector TSVECTOR,  -- 用于精确分词搜索
    created_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- 组合查询：先用tsvector精确匹配，再用pg_bigm模糊补充
WITH exact_match AS (
    SELECT id, title, ts_rank(search_vector, query) as rank
...
fuzzy_match AS (
    SELECT id, title, pg_bigm.similarity(title, 'PostgreSQL database') as score
    FROM articles
    WHERE title LIKE '%PostgreSQL%' OR title LIKE '%database%'
)
SELECT COALESCE(e.id, f.id) as id,
       COALESCE(e.title, f.title) as title,
       COALESCE(e.rank, 0) + COALESCE(f.score, 0) as combined_score,
       CASE WHEN e.id IS NOT NULL THEN '精确' ELSE '模糊' END as match_type
FROM exact_match e
FULL OUTER JOIN fuzzy_match f ON e.id = f.id
ORDER BY combined_score DESC
LIMIT 20;
```

##### 7.4、ParadeDB pg_search的进步与限制
...
###### ZomboDB 的限制与注意事项
- ES 写入失败会导致 PG 事务失败。如果 ES 宕机或网络不通，PG 的 INSERT/UPDATE/DELETE 会直接报错回滚。
- ES schema 由 ZomboDB 自动管理，无法自由控制 ES 的 mapping（如分词器、字段类型等）。
- 版本兼容性，ZomboDB 仅支持 PostgreSQL 12–16。
...
##### 3.3 CDC
PG 的 CDC（Change Data Capture） 是指对 PostgreSQL 数据库中的数据变更（Insert/Update/Delete）进行实时捕获和同步 的一类技术和机制。它通常用于 数据同步、消息驱动架构、审计、缓存更新、实时 ETL、数据湖入湖 等场景。

PostgreSQL CDC 常用工具对比：

| 工具                     | 原理         | 优点                 | 缺点         |
| ---------------------- | ---------- | ------------------ |------------|
| **Debezium**           | WAL → 逻辑解码 | 功能最强，Kafka 生态，社区活跃 | 配置相对复杂     |
| **pg_recvlogical**（官方） | 逻辑解码       | 官方支持，轻量            | 功能简单，需自写适配 |
| **wal2json**           | 插件         | 输出 JSON，最常用        | 性能一般       |
| **pgoutput**（官方）       | 插件         | 性能最好               | 输出结构复杂     |
| **Bottled Water**      | WAL 逻辑解码   | 高性能                | 维护较少       |
| **Maxwell for PG**（新兴） | WAL 逻辑解码   | 类似 MySQL Maxwell   | 不够成熟       |
