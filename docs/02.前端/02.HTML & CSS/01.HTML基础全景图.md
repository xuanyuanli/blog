...
## 行内元素和块级元素
在进行学习的过程中，你首先会遇到两个常识性的概念，就是行内元素和块级元素。    
其实在HTML 5规范中并没有这行内和块级的概念，而在HTML 4.01标准中出现的出现过。HTML 5中用了更复杂的 [内容类别](https://developer.mozilla.org/en-US/docs/Web/Guide/HTML/Content_categories) 来替代。    
由于行内和块级概念的简洁性，它们在实践中确实也更为流行。       
这两个概念并不单独属于HTML的内容，会涉及到CSS的display属性，涉及到浏览器的元素排版原理。    
简单来说，它们的主要区别是：  
- 块级元素：独占一行，对宽高的属性值生效，有盒子模型。不给宽度，块级元素就默认为浏览器的宽度，即就是100%宽
- 行内元素：可以多个标签存在一行，对宽高属性值不生效，完全靠内容撑开宽高


假设你用纯HTML编程，不涉及到CSS的话，那么理解行内和块级的概念有助于对知识的掌握。      
比如下面的效果：  
::: demo [vanilla]
```html
<html>
...
a		超链接，rel属性值有：alternate、author、help、license、next、prev、search、tag、bookmark、nofollow、noopener、noreferrer、opener
b/u		表示一段文本
em		强调
i			科学术语、外文词语
s			不精确、不正确的内容
...
sup/sub		上标或下标
br/wbr		换行或适合换行
code/var/samp/kdb		计算机代码、程序输出、变量或用户输入
abbr		缩写
dfn		术语
q			引用
cite		引用其他作品的标题
ruby/rt/rp	东亚语言中的注音符号
bdo		一段内容的文本方向
bdi		
span		
mark		另一段上下文有关的内容
