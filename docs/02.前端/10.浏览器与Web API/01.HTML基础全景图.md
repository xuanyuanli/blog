...
- 具体为佳，一以贯之。标签太多，语义不明？你喜欢`div`布局，没关系，一以贯之
...
## 九、HTML5新特性
HTML5引入了许多新特性，极大地增强了Web应用的能力。以下是主要的新特性：

### 1、语义化标签
前面章节已介绍的语义化标签都是HTML5引入的，包括：
- `header`、`footer`、`nav`、`article`、`section`、`aside`、`main`等
- 这些标签让页面结构更清晰，有利于SEO和可访问性
...
### 3、SVG支持
虽然SVG不是HTML5首创，但HTML5正式将其纳入标准，可以直接在HTML中嵌入`<svg>`元素（前面已介绍）。

### 4、音频和视频
原生支持`<audio>`和`<video>`标签（前面已介绍），无需依赖Flash等插件。

### 5、地理定位API
通过JavaScript获取用户地理位置：
```javascript
...
### 10、表单增强
前面介绍的表单新类型都是HTML5引入的：
- 新的`input`类型：`email`、`url`、`tel`、`date`、`time`、`color`、`range`等
- 新的表单属性：`placeholder`、`required`、`pattern`、`autofocus`等
- `<datalist>`元素提供输入建议

### 11、离线应用
HTML5支持应用缓存（Application Cache），但已被Service Worker取代：
```javascript
...
- **Page Visibility API**：检测页面是否可见
  ...
  document.addEventListener('visibilitychange', () => {
    if (document.hidden) console.log('页面被隐藏');
  });
  ```
...
当然，HTML只是千层之台的第一步，后续还有`CSS`和`JS DOM`相关的内容，有了`CSS`，就有了神笔马良；而有了`JS DOM`，就有了展示动态内容的能力，插上了飞翔的翅膀。  
任重而道远，让你我继续前进。
