...
| **读性能** | 优（直接读取目标版本） | 良（需要回滚构造） | PG读取更快 |
| **写性能** | 良（需要vacuum清理） | 优（原地更新） | MySQL写入稍快 |
...
| **LATERAL JOIN** | 完整支持 | 不支持 | 动态关联查询     | 复杂关联场景的唯一解 |
...
```sql
-- MySQL：需要多次查询或存储过程
-- 无法在一个SQL中优雅实现复杂的层次遍历
```

**业务价值**：PostgreSQL可以一条SQL处理任意深度的层次数据，MySQL需要程序循环查询。

##### 2.3、LATERAL JOIN：动态关联的强大武器

`LATERAL JOIN`让子查询可以引用左侧表的字段，实现"依赖关联"。这是PostgreSQL独有的特性，MySQL完全不支持。

**核心价值**：解决"为每个XX找到对应的最新/最多/最少的YY"这类问题，一条SQL搞定。
...
```sql
-- MySQL：无法实现，只能用子查询获取一条记录
SELECT
    c.customer_name,
    (SELECT order_date FROM orders WHERE customer_id = c.customer_id ORDER BY order_date DESC LIMIT 1) as latest_date,
    (SELECT amount FROM orders WHERE customer_id = c.customer_id ORDER BY order_date DESC LIMIT 1) as latest_amount
FROM customers c;
-- 性能差，且只能获取一条记录
```
...
user_stats AS (
    SELECT
        ...
        -- 地理分析
        STRING_AGG(DISTINCT city, ', ') as visited_cities,
...
SELECT
    ...
    -- 设备偏好
    CASE
        WHEN mobile_usage > desktop_usage * 2 THEN '移动优先'
        WHEN desktop_usage > mobile_usage * 2 THEN '桌面优先'
        ELSE '多设备用户'
    END as device_preference,

    -- 地理活跃度
    CASE
        WHEN unique_cities > 3 THEN '高流动性用户'
        WHEN unique_cities > 1 THEN '中等流动性用户'
        ELSE '固定位置用户'
    END as mobility_level,

    visited_cities,
    DATE_PART('day', NOW() - last_activity) as days_since_last_activity
FROM user_stats
ORDER BY total_spent DESC, login_count DESC;
```
...
-- 创建复合索引：数组 + 其他字段
CREATE INDEX idx_articles_author_tags ON articles USING GIN(author_id, tags);
...
- **时间段管理**：预订系统、排班管理、活动时间
- **价格区间**：促销价格、分层定价、成本区间
- **库存范围**：库存上下限、安全库存、预警范围
- **地理范围**：经纬度范围、海拔高度、温度区间
- **版本控制**：生效时间范围、版本有效期
...
SELECT
    ...
    -- 计算两点间距离
    position(l1.position, l2.position) as distance_degrees
