...
> **免责声明**：本章节中的性能数据仅供参考，实际性能表现会因硬件配置、数据规模、应用场景等因素而有所不同。建议读者在实际项目中进行针对性的基准测试。

#### 1、测试环境说明

**标准测试环境配置**
- **硬件配置**：Intel Xeon Gold 6248R (32核心)，128GB DDR4 内存，NVMe SSD 1TB
- **操作系统**：Ubuntu 22.04 LTS
- **数据库版本**：MySQL 8.0.35，PostgreSQL 15.4
- **测试工具**：pgbench, sysbench, TPC-H, 自定义脚本
- **数据规模**：1000万用户表，5000万订单表，100GB TPC-H数据集

#### 2、索引技术对比

PostgreSQL在索引技术方面具有显著优势，支持`GiST`、`GIN`、`BRIN`、部分索引、表达式索引等MySQL不具备的高级索引类型（详见前文索引章节）。以下通过实际测试展示关键差异：
...
-- 1000万记录，单线程测试：
-- MySQL: 0.08ms 平均响应时间
-- PostgreSQL: 0.12ms 平均响应时间 (MySQL快33%)
...
-- 测试结果：
-- MySQL: 45秒 (22,222 rows/sec)
-- PostgreSQL: 52秒 (19,230 rows/sec) (MySQL快14%)
```
...
| CPU核数 | MySQL TPS | PostgreSQL TPS | PG领先幅度 |
|---------|-----------|----------------|-----------|
| 4核     | 15,000    | 16,500         | +10%      |
| 8核     | 28,000    | 32,000         | +14%      |
| 16核    | 52,000    | 61,000         | +17%      |
| 32核    | 95,000    | 115,000        | +21%      |
| 64核    | 165,000   | 210,000        | +27%      |

**结论**：PostgreSQL在多核扩展性上表现更优，核数越多优势越明显。
...
| 锁等待分析 | 性能模式 | pg_locks | PostgreSQL更直观 |
...
> **评分说明**：基于标准测试环境，满分100分，评分考虑性能、稳定性、可扩展性等因素

| 测试场景 | MySQL得分 | PostgreSQL得分 | 优势方 | 性能差距 |
|----------|-----------|----------------|--------|----------|
| 简单点查询 | 92 | 85 | MySQL | 约15% |
| 高并发简单读取 | 90 | 82 | MySQL | 约10% |
| 批量插入操作 | 88 | 83 | MySQL | 约14% |
| OLTP事务处理 | 85 | 92 | PostgreSQL | 约15% |
| OLAP分析查询 | 75 | 95 | PostgreSQL | 约60% |
| 混合工作负载 | 80 | 88 | PostgreSQL | 约12% |
| JSON数据处理 | 70 | 95 | PostgreSQL | 约165% |
| 复杂子查询 | 72 | 90 | PostgreSQL | 约300% |
...
pgbench -c 50 -j 10 -T 300 -r testdb  # 50并发，运行5分钟
