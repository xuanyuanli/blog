...
启动Logstash容器：
```bash
docker run --name logstash -e "ES_JAVA_OPTS=-Xms512m -Xmx4096m" --restart=on-failure:3 --net=host -d -v /etc/logstash/pipeline/:/usr/share/logstash/pipeline/ -v /etc/logstash/logstash.yml:/usr/share/logstash/config/logstash.yml docker.elastic.co/logstash/logstash:7.3.1
```
