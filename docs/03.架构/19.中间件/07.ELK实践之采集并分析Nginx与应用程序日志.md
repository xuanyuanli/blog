...
启动Logstash容器：
```bash
docker run --name logstash -d -v /etc/logstash/pipeline/:/usr/share/logstash/pipeline/ docker.elastic.co/logstash/logstash:7.3.1
```
...
- `%{NUMBER:upstream_response_time}?`：匹配并提取数字，将其作为 `upstream_response_time` 字段。这里的问号表示该字段是可选的。
- `%{DATA:host}`：匹配并提取任意非空白字符序列，将其作为 `host` 字段。
