...
1. Redisearch：一个全文搜索引擎模块，允许您在 Redis 中进行复杂的全文搜索，支持模糊匹配、数值过滤和地理位置查询。它还提供了索引维护和实时查询的功能。
   官方网站：https://redisearch.io/

2. RediSQL：一个允许您在 Redis 内执行 SQL 查询的模块。它使用 SQLite 作为后端存储引擎，提供了灵活的数据查询和处理功能。
   官方网站：https://github.com/RedBeardLab/rediSQL

3. RedisGraph：一个基于图数据库的 Redis 模块，支持基于图形的数据查询和操作。它使用图结构来存储数据，并支持 Cypher 查询语言进行高效的图查询。
   官方网站：https://redisgraph.io/

4. RedisBloom：一个提供概率数据结构的 Redis 模块，包括布隆过滤器、计数布隆过滤器、顶点草图和空间草图。这些数据结构可以用来实现内存高效的去重、统计和近似查询等功能。
   官方网站：https://redisbloom.io/

5. RedisJSON：一个提供原生 JSON 数据类型支持的 Redis 模块，允许在 Redis 中存储、更新和查询 JSON 数据。它支持 JSONPath 和 JSON API，使得操作 JSON 数据变得更加简便。
   官方网站：https://redisjson.io/

6. RedisTimeseries：一个为 Redis 提供时间序列数据结构的模块，用于存储、查询和处理时间序列数据。它支持数据压缩、降采样、聚合和查询等功能。
   官方网站：https://redistimeseries.io/

7. RedisGears：一个 Redis 的服务器端脚本执行引擎，支持 Python 和其他语言编写的脚本。它提供了事件驱动的编程模型，允许在 Redis 服务器上运行复杂的逻辑。
   官方网站：https://redisgears.io/
