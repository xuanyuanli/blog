...
*   无需发明自定义的消息协议和消息格式。
*   STOMP客户端可用，包括Spring Framework中的 <a href="client.html" class="xref page">Java客户端</a>。
...
第二种方法是创建端到端集成测试。为此，您需要在嵌入模式下运行WebSocket服务器，并作为WebSocket客户端连接到该服务器，该客户端发送包含STOMP帧的WebSocket消息。[股票投资组合][https://github.com/rstoyanchev/spring-websocket-portfolio/tree/master/src/test/java/org/springframework/samples/portfolio/web]示例应用程序的测试也通过使用Tomcat作为嵌入式WebSocket服务器和用于测试目的的简单STOMP客户端来演示了这种方法。
