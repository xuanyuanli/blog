...
- **Spring Data增强**： 支持MongoDB 3.0的聚合框架和GridFS，以及Neo4j 2.2的路径查询。
- **测试工具改进**：`MockMvc`支持JSONP响应验证，适配前后端分离场景。`@SpringBootTest`引入`@AutoConfigureMockMvc`，简化MockMvc配置。
...
**主要特性**：
- **Spring Boot集成**：`@SpringBootApplication`注解简化Spring Boot配置。
...
**主要特性**：
- **响应式数据访问**：支持R2DBC（如PostgreSQL异步驱动）。
- **Kotlin 1.2支持**：优化Kotlin开发体验，增强DSL功能。
...
- **响应式改进**： WebFlux支持多部分表单的流式处理（`PartEvent` API）。`WebClient`集成JDK HttpClient，提供更高效的非阻塞HTTP客户端。
...
- **云原生增强**： 引入`JdbcClient`（R2DBC的JDBC变体），简化关系型数据库的响应式操作。 支持Kubernetes的`@ServiceBinding`注解，自动注入服务绑定配置。
...
**主要特性**：
- **并行Bean初始化**： 引入`@Bean(bootstrap = BACKGROUND)`，支持异步初始化非关键Bean，启动时间平均缩短30%。 通过`BeanFactory`的`parallelInit`配置项，自动检测无依赖的Bean并并行加载。
- **函数式编程深化**： 扩展`RouterFunction`支持路径模式匹配（如`/api/{version}/users`）。 提供`FunctionalEndpoint`接口，简化函数式端点的参数校验和异常处理。
- **云原生优化**： 整合Quarkus的`@ConfigMapping`，支持类型安全的配置绑定。 增强Kubernetes的`ConfigMap`和`Secret`动态刷新机制。
- **性能与稳定性**： 优化AOP代理生成，减少反射调用开销。 引入`CircuitBreaker`注解，内置Hystrix兼容的熔断器实现。
- **其他改进**： 移除`JmxAutoConfiguration`，推荐使用Micrometer进行监控。 支持Java 21的`Stream.iterate`无限流优化。
