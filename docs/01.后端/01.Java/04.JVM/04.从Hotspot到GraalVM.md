...
然而，Substrate VM在某些功能和兼容性方面可能存在局限性。

## 提前编译AOT（Ahead-of-Time）

### 1 AOT编译的概念与优势

AOT编译是将源代码或字节码直接编译成本地机器代码，而不是在运行时通过即时编译（JIT）生成。AOT编译具有以下优势：

1. 更快的启动时间，因为程序在运行时无需额外的编译过程。
2. 更低的内存占用，因为运行时不需要额外的编译器资源。
3. 更好的确定性，因为程序运行的性能不受运行时编译影响。

### 2 AOT编译与JIT编译的权衡

尽管AOT编译具有一定的优势，但与JIT编译相比，也存在一些权衡：

1. AOT编译可能导致更大的二进制文件，因为所有代码都被编译成本地机器代码。
2. AOT编译可能无法充分利用运行时信息进行优化，因此在某些场景下可能导致性能下降。
3. AOT编译过程可能需要较长的时间和计算资源。

### 3 AOT编译的局限性

AOT编译在某些功能和兼容性方面可能存在局限性，主要包括以下方面：
- 反射
- 动态类加载
- 动态代理
- Instrument

因此，在使用AOT编译时，开发者需要权衡这些局限性以及AOT编译带来的性能优势。
...
2. JMX和JVMTI等运行时监控和诊断功能可能受到限制，因为原生映像不包含完整的JVM。
3. 反射和动态类加载等功能可能受到限制，因为原生映像在编译时已确定了所有的类和方法。
4. 与某些第三方库和框架的兼容性可能受到影响，特别是动态代理库，比如Cglib等。

要兼容Instrument和JVMTI会有些复杂，比如使用Truffle框架开发语言插件来替代Instrument和JVMTI。 

对于JMX的兼容，则需要在构建原生映像时添加`-H:+AllowJMXService`选项：

```bash
native-image -H:+AllowJMXService -cp . com.example.Main
```

使用这个选项会允许原生映像使用部分JMX功能，如MBeanServer、MBean注册和查询等。但是，请注意，即使启用了这个选项，原生映像中的JMX支持仍然有以下限制：

1. 不支持JMX远程连接：原生映像中的JMX功能仅限于在本地使用。你无法像在常规JVM中那样使用JMX远程连接来监控和管理原生映像。
2. 可能缺少某些内置MBeans：原生映像可能不包含所有在常规JVM中可用的内置MBeans。因此，在原生映像中，某些JMX客户端和工具可能无法正常工作。
