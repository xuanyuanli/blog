...
```
结果是不会输出任何东西，即使`Runnable`任务重抛出了异常。为啥会出现这种情况呢？感兴趣的同学可以自己研究下源码，主要涉及的类有`FutureTask`和`RunnableAdapter`，主要看`FutureTask`的run()方法即可了解原因。  
不抛异常可是非常危险，为了解决这个问题，建议像线程池提交`Runnable`类型的任务使用`execute()`方法比较好！  
