...
下面列出一些提高效率的便利方法：
- subtract&emsp;取得差集
- disjunction&emsp;获得彼此差集之后的并集
- intersection&emsp;取得交集
- union&emsp;取得并集
- containsAny&emsp;包含任何
- permutations&emsp;返回列表的所有排列组合，大小是列表大小的阶乘级别，注意性能和内存问题
...
List的扩展，主要有如下类：
- CursorableLinkedList&emsp;提供了可修改的迭代器。在JDK的List.listIterator中提供了类似功能
- FixedSizeList&emsp;固定大小的List，add、remove、clear和retain操作是不被支持的，set方法是允许的但是不会影响列表大小
- GrowthList&emsp;可以使其在因set或add操作造成索引超出异常时无缝的增加列表长度，可以避免大多数的IndexOutOfBoundsException
- LazyList&emsp;可以自动增长，但只发生在get时--允许指定一个Factory获得超出索引位置的值
- PredicatedList&emsp;添加元素时进行校验
- SetUniqueList&emsp;不允许重复元素的列表
- TransformedList&emsp;在set和add时对元素进行转换后再添加
- TreeList&emsp;实现了一个快速插入、删除，同时查询性能也可观的List
...
Set的扩展有如下类：
- CompositeSet&emsp;多个Set的组合视图
- 剩余的Predicated、Transformed、Unmodifiable和List一样，不赘言

--- 

Map的扩展有如下类：
- CaseInsensitiveMap&emsp;大小写不敏感的Map，会把键都转换为小写
- CompositeMap&emsp;多个Map的组合视图
- DefaultedMap&emsp;键不存在，返回默认对象
- FixedSizeMap&emsp;固定大小的Map
- IdentityMap&emsp;匹配键值不是通过==，而是equals
- MultiKeyMap&emsp;允许多个键关联到一个值上
- MultiValueMap&emsp;允许多个值关联到一个键上
- BidiMap&emsp;双向Map，可以通过值查找键。主要实现类是DualHashBidiMap、TreeBidiMap
- LRUMap&emsp;固定大小Map，容器满了之后删除最近最少使用（Least Recently Used）的元素
...
- Lists.partition&emsp;分割列表。返回连续子列表，每个子列表大小系统（最后一个可能更小）
- Sets.subSet&emsp;提取范围内的子视图
- Maps.subMap&emsp;提取范围内的子视图

## 并发集合
前面说道不推荐使用Collections.synchronized系列方法，原因是它内部只是提供了一个简单的互斥操作，在高并发情况下，并不十分高效。  
Java 5之后利用CAS指令（乐观锁）提供了更高效的并发容器，它们分别是：
- ConcurrentLinkedQueue&emsp;高并发环境下性能最好的队列
- CopyOnWriteArrayList&emsp;只在写时加锁的ArrayList，采用写时复制的思想，读性能大幅提升
- BlockingQueue&emsp;一个阻塞式的数据共享通道，参考：[阻塞队列BlockingQueue](../597089/)。他有三个主要实现：
  - ArrayBlockingQueue&emsp;数组支持的有界队列
  - LinkedBlockingQueue&emsp;链表支持的可选有界队列
  - LinkedBlockingDeque&emsp;链表支持的双向可选有界队列
- ConcurrentMap&emsp;线程安全的map，主要实现有：
  - ConcurrentHashMap&emsp;高并发哈希表
  - ConcurrentSkipListMap&emsp;跳表，类似LinkedHashMap，提供有序的遍历。类似于平衡树，特点是插入和删除只需要对局部数据进行操作即可。平衡树的话需要全局锁，而跳表只需要部分锁。
