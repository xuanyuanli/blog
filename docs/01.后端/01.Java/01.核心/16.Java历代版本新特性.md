...
## 21（LTS）
2023年9月[发布](https://openjdk.org/projects/jdk/21/)。

* JEP 430: [字符串模板（预览）](https://www.baeldung.com/java-21-string-templates)
* JEP 431: [顺序集合](https://www.baeldung.com/java-21-sequenced-collections)
* JEP 439: 分代ZGC
* JEP 440: 记录类的模式匹配
* JEP 441: switch的模式匹配
  ```java
...
* JEP 444: [虚拟线程](https://mp.weixin.qq.com/s/yyApBXxpXxVwttr01Hld6Q)
* JEP 445: [无名类和实例主方法（预览）](https://www.baeldung.com/java-21-unnamed-class-instance-main)
* JEP 446: 作用域值（预览）
...
## 22
2024年3月[发布](https://openjdk.org/projects/jdk/22/)。
...
## 23
2024年9月[发布](https://openjdk.org/projects/jdk/23/)。

* JEP 455: 支持instanceof和switch中的原始类型（预览）。比如`if(i instanceof byte b)`，或者`switch(i){ case int i -> ... ;}`
...
* JEP 476: 模块导入声明（预览）。可以直接导入模块：`import module java.base`，这样会导入模块下的所有类
