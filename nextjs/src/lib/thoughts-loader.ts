...
/**
 * 解析单个 Markdown 文件
 * 优先使用 frontmatter 中的 path 字段作为 URL slug，
 * 未设置时回退到文件名（去掉数字前缀和 .md 后缀）
 */
function parseThoughtFile(filename: string): Thought {
  ...
  const slug = data.path ? String(data.path) : slugFromFilename(filename);
...
export function getAllThoughtSlugs(): string[] {
  return getAllThoughts().map((t) => t.slug);
}
