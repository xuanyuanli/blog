...
用途：ETF 周线轮动策略常驻服务 — 每周最后一个交易日 14:30（上海时间）对比标的池（沪深300ETF / 创业板50ETF / 纳指100ETF，共 3 只）近 4 个交易周累计涨幅，取最高者，为正则持有、为负则空仓；决策经 Server酱 推送，持仓状态写 `state.json`。内置回测。周K 拉取对 159949 等品种做多源回退（auto / 东财 / 腾讯 fqkline，兼容 `week`/`qfqweek`）。
...
node dist/cli.js backtest             # 回测沪深300+创业板50+纳指100
node dist/cli.js backtest --end 2024-12-31
node dist/cli.js combos               # 组合枚举（当前池 3 只）
node dist/cli.js backtest --start 2024-07-08
npm test
```

部署：`node blog-ops/bin/bops.js rotation` — 上传产物 + `Dockerfile` 到远程 `/data/apps/weekly-rotation/`，远程 `docker build` 后以容器 `weekly-rotation` 运行（`--restart unless-stopped`，`data/` 挂载为 `/data` 持久化 `state.json`）。通知密钥 `SERVERCHAN_SENDKEY` 优先取服务器环境变量（`-e` 注入），其次 bops 本地 conf（写远程 `.env` 后 `--env-file` 注入）。远程日志：`docker logs -f weekly-rotation`。
