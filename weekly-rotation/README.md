...
# 回测：对比两个标的池——5 只（全部）vs 4 只（不含成立较晚的科创芯片ETF）
...
回测口径：以周收盘价近似周五 14:30 执行价，逐周复利，不计费率与滑点；不指定 `--start` 时每个池的起点取池内最晚成立 ETF 的上市周（5 只池受科创芯片ETF 限制只有 2025-05 起约 59 周，4 只池可回溯到 2020-12 约 283 周）。指标含累计收益、年化、最大回撤、换仓次数、空仓周数，并输出各标的同期买入持有收益作基准。

参考结果（截至 2026-07-03）：5 只池 2025-05-09 起 59 周累计 +252.97%（最大回撤 16.60%）；4 只池 2020-12-18 起 283 周累计 +392.85%（年化 34.05%，最大回撤 22.61%）。两者区间长度差异大，年化不可直接对比。
