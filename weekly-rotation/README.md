...
实盘（daemon / once）与回测默认使用这三只。在「A 股宽基/成长 + 美股纳指」组合回测中，该池在 2016-08 起的长窗口里收益与卡玛均优于仅沪深300+创业板50，且回撤更低。
...
node dist/cli.js backtest --start 2016-08-19
...
参考结果（截至 2026-08-28，起点 2016-08-19）：

| 标的池 | 周数 | 累计收益 | 年化 | 最大回撤 | 卡玛 |
| --- | --- | --- | --- | --- | --- |
| 沪深300+创业板50+纳指100 | 512 | +1054.8% | 28.2% | 20.7% | 1.36 |
| 沪深300+创业板50（旧池） | 512 | +391.3% | 17.5% | 27.1% | 0.65 |

> 备注：曾对比过标普500、纳指科技等跨境 ETF。标普500 降波效果有、收益天花板不如纳指100；纳指科技（159509）2023-08 才上市，短窗数字好看但样本不足。均线卖出增强（MA5/MA10/MA20）长窗口全部跑输基础策略。故实盘定为沪深300+创业板50+纳指100。
...
若服务器上已有旧 `state.json` 且持仓不在当前池内，下次轮动会按「旧持仓 → 新池决策」记一次换仓/清仓，属预期行为。
